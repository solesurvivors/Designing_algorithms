#ifndef CADJANCENCYMATRIX_H
#define CADJANCENCYMATRIX_H
...
namespace cfg
{
   ...
   const double defaultFeromone = 0.01;
}
...
public:
   ...
   explicit CAdjancencyMatrix(int vertexCount, bool directed, bool noDeadends = true, int noConnectionValue = cfg::hasNoConnection, double defaultFeromone = cfg::defaultFeromone)
      : mVCount(vertexCount)
      , mIsDirected(directed)
      , mHasNoDeadends(noDeadends)
      , mNoConnectionValue(noConnectionValue)
      , mDefaultFeromone(defaultFeromone)
   {
      init(noConnectionValue, defaultFeromone);
   }

   explicit CAdjancencyMatrix(const std::string& fileName)
      : mVCount(0)
      , mNoConnectionValue(cfg::hasNoConnection)
      , mDefaultFeromone(cfg::defaultFeromone)
   {
      load(fileName);
   }
   ...
   /// @brief
   void addEdge(int idxFrom, int idxTo, int lenght, bool directed)
   {
       mData[idxFrom][idxTo] = { lenght, mDefaultFeromone };
       if (!directed) {
           mData[idxTo][idxFrom] = { lenght, mDefaultFeromone };
       }
       mVertexPower[idxFrom] += 1+int(directed);
       mVertexPower[idxTo] += 1+int(directed);
   }
   ...
   void generate(std::pair<int, int> vertexPowerRange, std::pair<int, int> edgeLenghtRange)
   {
      ...
      for (int i = 0; i < mVCount; ++i)
      {
         ...
         for (int j = 0; (j < vertexPowerAdd) && !whiteList.empty(); ++j)
         {
            ...
            mData[i][cVertex] = { len, mDefaultFeromone };
            if (!mIsDirected) {
               mData[cVertex][i] = { len, mDefaultFeromone };
            } else {
               const int hasConnection = mHasNoDeadends || (hasConnectionPowerGen() != 0);
               if (hasConnection) {
                  auto len_back = lenghtPowerGen();
                  mData[cVertex][i] = { len_back, mDefaultFeromone };
               }
            }
   ...
   void save(const std::string& fileName) const
   {
      ...
      saveFile << mDefaultFeromone << std::endl;
   ...
   bool load(const std::string& fileName)
   {
      ...
      saveFile >> mDefaultFeromone;

      init(mNoConnectionValue, mDefaultFeromone);
...
private:
   ...
   void init(int noConnectionValue, double defaultFeromone)
   {
...
public:
   ...
   /// @brief default value for feromone on the edge
   double mDefaultFeromone;
